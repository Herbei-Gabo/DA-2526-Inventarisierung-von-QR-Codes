...
Derzeit erfolgt die Inventarisierung von Objekten, Werkzeugen sowie IT-Hardware an unserer Schule mit einer herkömmlichen Excel-Tabelle. Ein Beispiel dafür wurde uns [zur Verfügung gestellt](https://github.com/Herbei-Gabo/DA-2526-Inventarisierung-von-QR-Codes/blob/main/Diplomarbeit/doc/HTLGesamtinventar_Monitorbeispiel.xlsx). Dabei werden Informationen wie Bereich, Objektbezeichnung, Lieferant, Seriennummer, Standort, Zuständigkeit und vieles Weitere manuell erfasst und gepflegt. Diese Form der Datenhaltung bringt jedoch zahlreiche Herausforderungen mit sich:
...
Damit sind kostenlose Alternativen für den angestrebten Anwendungsbereich ungeeignet, da sie mehr Aufwand erzeugen, als sie Nutzen bringen.

### Kostenpflichtige Alternativen

Im Bereich der kostenpflichtigen Softwarelösungen existieren mehrere Anbieter, die Inventarisierung speziell für Institutionen wie Schulen oder öffentliche Einrichtungen anbieten. Folgende Beispiele werden exemplarisch betrachtet:
...
Das aktuelle Ticketsystem der HTL Leoben läuft Open Source auf https://ticket.htl-leoben.at/, und wurde von MantisBT erstellt. Dabei melden sich jeweilige Schüler und Lehrer über das Schulnetzwerk, welches über LDAP läuft, an und können einen Defekt oder einen Einwand reklamieren. [@htl_leoben_ticket]

Die Idee, sich an dieses Ticketsystem anzuschließen, klingt einfacher gesagt als getan. Zuallererst muss gesagt werden, dass unsere Inventarisierungslösung nicht als Open Source veröffentlicht wird. Diese Lösung sollte ausschließlich der HTL Leoben dienen.

Eine weitere Herausforderung ist, an das bestehende LDAP-Netzwerk anzuschließen, um die Login-Funktion zu vereinfachen. Anstatt sich zu registrieren und neue Benutzer anzulegen, werden die Benutzer des aktuellen Ticketsystems hergenommen und per LDAP an das Loginsystem der Inventarisierung weitergeleitet. So kann zum Beispiel der Lehrer, der sich einmal in das Ticketsystem angemeldet hat, sich ohne weitere Probleme bei der Inventarisierung anmelden.

## Theoretische Umsetzung der QR-Code-Drucklösung

Zur praktischen Umsetzung der Inventarisierungslösung wurde eine Möglichkeit entwickelt, QR-Codes effizient, kostengünstig und systematisch auszudrucken. Dafür wird auf handelsübliche, selbstklebende Etiketten im Format 45,7 × 45,7 mm zurückgegriffen. Diese wurden von der Firma Avery Zweckform bezogen (Produktnummer 6251, vgl. [@avery_vorlage_6251]) und über Amazon erworben [@avery_amazon_etiketten].
